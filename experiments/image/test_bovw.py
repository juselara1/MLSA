...
def train_histogram(path, trials):
    names = ["Codebook", "Accuracy", "Precision", "F1", "Recall"]
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description='Train and evaluate all the BoVW representations')
    parser.add_argument("--repr", type=str, help="Representation to evaluate {histogram, tfidf}", default="histogram")
    parser.add_argument("--bovw_path", type=str, help="Path of the bag of visual words.", default="/tf/TCGA/bovw.h5")
    parser.add_argument("--trials", type=int, help="Number of trials", default=30)
    args = parser.parse_args()
    os.system("clear")
    
    if args.repr=="histogram":
        train_histogram(args.bovw_path, args.trials)
    elif args.repr=="tfidf":
        train_tfidf(args.bovw_path, args.trials)