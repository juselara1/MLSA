...
augmentation = {"preprocessing_function": tf.keras.applications.inception_v3.preprocess_input, "apply":True,
...
train_seq = ImageAugmentationSequenceH5("/tf/TCGA/images.h5", "ims_train", "y_train", num_classes=2,
                                        ...
                                        class_weighted=True, categorical=True, batch_size=64,
                                        augmentation = augmentation, shuffle=True)

augmentation = {"preprocessing_function": tf.keras.applications.inception_v3.preprocess_input, "apply":False}
...
inception_base = tf.keras.applications.inception_v3.InceptionV3(weights="imagenet",
                                                                input_tensor=input_tensor,
                                                                include_top=False)
pool = tf.keras.layers.GlobalAveragePooling2D()(inception_base.output)
...
callback = tf.keras.callbacks.ModelCheckpoint("inceptionV3.h5")
model.fit(train_seq, steps_per_epoch=len(train_seq), validation_data=val_seq, validation_steps=len(val_seq),
          epochs=20, callbacks=[callback], shuffle=False, use_multiprocessing=False)
